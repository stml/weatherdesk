...
**N.B.** The image will be black during local night time! At dawn and dusk, you will see the [terminator](https://en.wikipedia.org/wiki/Terminator_(solar)), as the sun's light arrives and then leaves the hemisphere. ~~(Unfortunately, the API has started returning non-alpha i.e. white-background images - working on a fix for this!)~~ FIXED!
